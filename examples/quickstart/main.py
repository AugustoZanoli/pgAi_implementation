...
# Embedding model used by both the vectorizer (in the database) and the query
# side (here in Python). Both MUST match so the vectors are comparable.
...
async def _find_relevant_chunks(query: str, limit: int = 1) -> List[WikiSearchResult]:
    """
    ...
    """
    # Generate the query embedding with the SAME Gemini model and dimensions
    # used by the vectorizer, otherwise the <=> comparison would be invalid.
...
async def run():
    ...
    # wait=True makes open() block until the initial connections are ready and
    # raises promptly if the database is unreachable, instead of retrying in the
    # background and emitting repeated "error connecting in 'pool-1'" warnings.
    ...
    try:
        ...
        results = await _find_relevant_chunks("Who is the father of computer science?")
        ...
        results = await _find_relevant_chunks("What is pgai?")
        ...
        relevant_chunks = await _find_relevant_chunks(query)
        context = "\n\n".join(
            f"{chunk.title}:\n{chunk.text}"
...
Answer:"""
        ...
        # Always close the pool so the process can exit cleanly instead of
        # hanging on leaked connections.
        await pool.close()
